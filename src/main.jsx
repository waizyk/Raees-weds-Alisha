...
function Album({ album, setAlbum, notify }) {
  ...
  const upload=async(e)=>{const files=[...e.target.files];if(!files.length)return;if(files.length>10){setError('Please upload no more than 10 photographs at a time.');return}setBusy(true);setError('');try{if(isSupabaseConfigured){if(!code||!uploader)throw new Error('Open the album with your invitation before uploading.');await uploadGuestPhotos(files,{code,uploadedBy:uploader});notify(`${files.length} ${files.length===1?'photograph':'photographs'} sent for approval`);}else{const additions=files.map(f=>({name:f.name,url:URL.createObjectURL(f)}));setAlbum(a=>[...a,...additions]);notify(`${files.length} ${files.length===1?'memory':'memories'} added`);}}catch(err){setError(err.message||'The photographs could not be uploaded.')}finally{setBusy(false);e.target.value=''}};
  return <PageHero eyebrow="OUR SHARED MEMORIES" title={<>The day, through<br/><em>your eyes.</em></>}>
    ...
    <div className="album-intro"><div><h3>Help us remember every moment</h3><p>From the dance floor to the quiet in-between moments—add your photographs here. Every upload is private until the couple approves it.</p>{isSupabaseConfigured&&<label className="album-uploader">Uploading as<input value={uploader} onChange={e=>setUploader(e.target.value)} placeholder="Your name"/></label>}{error&&<p className="form-error">{error}</p>}</div><label className={`button dark upload ${busy?'disabled':''}`}><ImagePlus size={17}/> {busy?'Uploading…':'Add photographs'}<input disabled={busy} type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" multiple onChange={upload}/></label></div>
    <div className="gallery-grid">
      {approved.map(p=><figure key={p.id}><img src={p.url} alt={p.caption||`Photograph shared by ${p.uploaded_by}`}/><figcaption>Shared by {p.uploaded_by}</figcaption></figure>)}
      {!isSupabaseConfigured&&album.map((p,i)=><figure key={i}><img src={p.url} alt={p.name}/><figcaption>Guest memory · awaiting approval</figcaption></figure>)}
      ...
      <label className="gallery-add"><Plus/><b>{busy?'Uploading…':'Add your memory'}</b><small>Up to 10 images · 15 MB each</small><input disabled={busy} type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" multiple onChange={upload}/></label>
    </div>
...
function PhotoApprovals({notify}){const [photos,setPhotos]=useState([]);const [busy,setBusy]=useState(true);const load=async()=>{setBusy(true);try{setPhotos(await listPhotoQueue())}catch(error){notify(error.message||'Could not load photographs')}finally{setBusy(false)}};useEffect(()=>{load()},[]);const decide=async(id,status)=>{try{await moderatePhoto(id,status);setPhotos(items=>items.map(p=>p.id===id?{...p,status}:p));notify(status==='approved'?'Photograph approved':'Photograph rejected')}catch(error){notify(error.message||'Could not update photograph')}};return <div className="photo-approvals"><div className="photo-approval-tools"><p>{photos.filter(p=>p.status==='pending').length} photographs awaiting review</p><button onClick={load}><RefreshCw/> Refresh</button></div>{busy?<div className="empty-list"><Camera/><b>Loading photographs…</b></div>:!photos.length?<div className="empty-list"><Camera/><b>No photographs yet</b><span>Guest uploads will appear here for approval.</span></div>:<div className="moderation-grid">{photos.map(p=><article key={p.id}><img src={p.url} alt={`Uploaded by ${p.uploaded_by}`}/><div><b>{p.uploaded_by}</b><small>{p.invitations?.guest_names} · {new Date(p.created_at).toLocaleDateString()}</small><Status status={p.status}/><div className="moderation-actions"><button disabled={p.status==='approved'} onClick={()=>decide(p.id,'approved')}><Check/> Approve</button><button className="reject" disabled={p.status==='rejected'} onClick={()=>decide(p.id,'rejected')}><X/> Reject</button></div></div></article>)}</div>}</div>}
