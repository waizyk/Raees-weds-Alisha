...
import {
  ...
  Download, Edit3, ExternalLink, Heart, ImagePlus, LayoutDashboard, Leaf, Link2,
...
function App() {
  ...
  useEffect(() => { if(isSupabaseConfigured) getHostSession().then(async session=>{if(session){setAdmin(true);try{const profile=await getHostProfile();if(profile?.display_name)setHostName(profile.display_name);}catch{setHostName(session.user?.email?.split('@')[0]||'Wedding host');}try{const details=await getEventDetails();if(details?.settings)setEventInfo(details)}catch{}}}); }, []);
...
function FamilyTree() {
  ...
  const [zoom,setZoom]=useState(1); const [pan,setPan]=useState({x:0,y:0}); const [search,setSearch]=useState(''); const [selected,setSelected]=useState(null);
  ...
  const [suggestion,setSuggestion]=useState({suggestedBy:savedInvite?.guestNames||'',suggestedName:'',relationship:'',placement:'',details:''});
  ...
  const generations=useMemo(()=>[...new Set(tree.members.map(m=>m.generation))].sort((a,b)=>a-b),[tree]);
  const generationName=(g)=>g<=-3?'Great-grandparents':g===-2?'Grandparents':g===-1?'Parents':g===0?'Raees & Alisha':g===1?'Children':g===2?'Grandchildren':'Family & friends';
  const matches=(m)=>!search||`${m.name} ${m.relationship_label} ${m.details}`.toLowerCase().includes(search.toLowerCase());
  const changeZoom=(amount)=>setZoom(z=>Math.max(.62,Math.min(1.65,Number((z+amount).toFixed(2)))));
  const pointerDown=e=>{if(e.target.closest('button,.tree-member-card,input'))return;drag.current={x:e.clientX,y:e.clientY,pan};e.currentTarget.setPointerCapture(e.pointerId)};
  const pointerMove=e=>{if(!drag.current)return;setPan({x:drag.current.pan.x+e.clientX-drag.current.x,y:drag.current.pan.y+e.clientY-drag.current.y})};
  ...
  return <PageHero eyebrow="OUR PEOPLE" title={<>Two families,<br/><em>one growing tree.</em></>}>
    <div className="tree-credit"><TreePine/><div><h3>The Khan–Ahmed family & friends tree</h3><p>Search for someone, tap a card for details, or zoom out to see earlier generations.</p></div></div>
    <div className="interactive-tree-shell">
      <div className="tree-toolbar"><div className="tree-search"><Search/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search family members…"/></div><div className="zoom-controls"><button onClick={()=>changeZoom(-.15)} aria-label="Zoom out"><ZoomOut/></button><span>{Math.round(zoom*100)}%</span><button onClick={()=>changeZoom(.15)} aria-label="Zoom in"><ZoomIn/></button><button onClick={()=>{setZoom(1);setPan({x:0,y:0})}}>Reset</button></div></div>
      <div className="tree-hint"><Move/> Drag to move · Zoom out for parents and grandparents · Zoom in for details</div>
      <div className="tree-viewport" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp}>
        {loading?<div className="tree-loading"><TreePine/><b>Growing the family tree…</b></div>:error&&!tree.members.length?<div className="tree-loading"><LockKeyhole/><b>{error}</b></div>:<div className={`tree-board ${zoom>1.15?'show-details':''}`} style={{transform:`translate(calc(-50% + ${pan.x}px),${pan.y}px) scale(${zoom})`}}>
          <div className="tree-trunk"/>{generations.map(g=><section className="generation-row" key={g}><h4>{generationName(g)}</h4><div>{tree.members.filter(m=>m.generation===g).map(m=><button key={m.id} className={`tree-member-card side-${m.side} ${matches(m)?'match':'dim'} ${selected?.id===m.id?'selected':''}`} onClick={()=>setSelected(m)}><span className="member-avatar">{m.photo_url?<img src={m.photo_url} alt=""/>:m.name.split(' ').map(x=>x[0]).slice(0,2).join('')}</span><b>{m.name}</b><small>{m.relationship_label}</small>{m.details&&<p>{m.details}</p>}</button>)}</div></section>)}</div>}
      </div>
    </div>
...
function Admin({ rsvps, setRsvps, notify, previewInvite, hostName, privacy, setPrivacy, eventInfo, setEventInfo, onSignOut }) {
  ...
  const exportCsv=()=>downloadCsv([['Respondent','Email','Invited names','Invite code','Guests','Status','Meal preference','Dietary requirements / allergies / notes','Table','Checked in'],...rsvps.map(r=>[r.name,r.email,r.invitedAs||r.household,r.inviteCode||'',r.guests,r.status,r.meal,r.note||'',r.table||'',r.checkedIn?'Yes':'No'])],'raees-alisha-rsvps.csv','Complete RSVP list downloaded');
  const exportDietary=()=>downloadCsv([['Respondent','Invited names','Guests attending','Meal preference','Dietary requirements / allergies','Email','RSVP status'],...rsvps.filter(r=>r.status!=='declined').map(r=>[r.name,r.invitedAs||r.household,r.guests,r.meal,r.note||'None provided',r.email,r.status])],'raees-alisha-dietary-requirements.csv','Dietary requirements report downloaded');
...
function GuestTable({rsvps,search,setSearch,update,removeDeclined}){return <div className="dash-card table-card"><div className="table-tools"><div><Search/><input placeholder="Search guests, meals or dietary needs" value={search} onChange={e=>setSearch(e.target.value)}/></div><span>{rsvps.length} responses</span></div><div className="dietary-callout"><Leaf/><div><b>Dietary requirements</b><span>Meal choices and every allergy or dietary note appear below and are included in both CSV exports.</span></div></div><div className="responsive-table"><table><thead><tr><th>Guest / household</th><th>Party</th><th>Meal & dietary requirements</th><th>Status</th><th>Actions</th></tr></thead><tbody>{!rsvps.length&&<tr><td colSpan="5"><div className="empty-table"><Users/><b>No guest responses yet</b><span>New RSVPs will be ready for approval here.</span></div></td></tr>}{rsvps.map(r=><tr key={r.id}><td><b>{r.name}</b><small>{r.household}</small><small>{r.email}</small></td><td>{r.guests}</td><td><div className="dietary-details"><b>{r.meal}</b><span>{r.note||'No dietary requirements provided'}</span></div></td><td><Status status={r.status}/></td><td>{r.status==='pending'?<div className="row-actions"><button onClick={()=>update(r.id,{status:'approved'})}><Check/> Approve</button><button className="reject" onClick={()=>update(r.id,{status:'declined'})}><X/> Decline</button></div>:<div className="row-actions"><button className="reset" onClick={()=>update(r.id,{status:'pending'})}>Change</button>{r.status==='declined'&&<button className="delete-response" onClick={()=>removeDeclined(r)}><Trash2/> Delete</button>}</div>}</td></tr>)}</tbody></table></div></div>}
...
function FamilyTreeEditor({notify}){
  ...
  const [data,setData]=useState({members:[],links:[]}); const [suggestions,setSuggestions]=useState([]); const [form,setForm]=useState(blank); const [link,setLink]=useState({from_member_id:'',to_member_id:'',link_type:'parent'}); const [suggestionSource,setSuggestionSource]=useState(null); const [busy,setBusy]=useState(true);
